...
private let log = Logger(subsystem: "com.taylor.audio.VXFissionExtension", category: "AudioUnitViewController")

@MainActor
public class AudioUnitViewController: AUViewController, AUAudioUnitFactory {
    var audioUnit: AUAudioUnit?
    
    var hostingController: HostingController<VXFissionExtensionMainView>?
	...
	*/
	...
	nonisolated public func createAudioUnit(with componentDescription: AudioComponentDescription) throws -> AUAudioUnit {
		return try DispatchQueue.main.sync {
			...
			audioUnit = try VXFissionExtensionAudioUnit(componentDescription: componentDescription, options: [])
			
			guard let audioUnit = self.audioUnit as? VXFissionExtensionAudioUnit else {
				log.error("Unable to create VXFissionExtensionAudioUnit")
				return audioUnit!
			}
			...
			audioUnit.setupParameterTree(VXFissionExtensionParameterSpecs.createAUParameterTree())
    ...
    private func configureSwiftUIView(audioUnit: AUAudioUnit) {
        ...
        let content = VXFissionExtensionMainView(parameterTree: observableParameterTree)
